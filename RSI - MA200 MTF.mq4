...
int init()
   ...
   string shortName  = "RSI-MA200 MTF ("+TimeFrameToString(TimeFrame)+","+RSIPeriod+","+PriceTypeToString(PriceType);
...
int start()
   ...
   if (alertsOn)
      ...
      if (RSIBuffer[0]>overBought && RSIBuffer[1]<overBought) 
         {
         doAlert(overBought+" RSI - MA200 line crossed up");
         }
      if (RSIBuffer[0]<overSold   && RSIBuffer[1]>overSold)
         {
         doAlert(overBought+" RSI - MA200 line crossed down");
         }
      }
   return(0);
   }
...
void doAlert(string doWhat)
   ...
   if (previousAlert != doWhat || previousTime != Time[0]) 
      ...
      if (alertsEmail)
         {
         SendMail(StringConcatenate(Symbol(),"RSI - MA200 crossing"),message);
         }
      }        
   }
